...
def create_common_parameter_list(
            description_string="This tool can be used to perform system management via Redfish.", prog_string=None, example_string=None
            ):
...
def parse_parameter(args):
    ...
    if not config_ini_info['ip'] or not config_ini_info['user'] or not config_ini_info['passwd']:
        sys.stderr.write(
                        "BMC connect information (ip/username/password) is needed. Please provide them by command line -i,-u,-p or configuration file (default config.ini)."
                        )
        sys.exit(1)
